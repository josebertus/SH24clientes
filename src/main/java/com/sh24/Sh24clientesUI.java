...
import com.servihogar.custom.general.frontend.CliPantallaBusquedaExpedientes;
import com.servihogar.custom.general.frontend.CliPantallaConsultaExpediente;
import com.servihogar.custom.general.server.jdbc.PAC_SHWEB_CLIENTES;
import com.servihogar.custom.general.server.jdbc.PAC_SHWEB_LISTAS;
...
public class Sh24clientesUI extends UI {
	...
	private CliCustomPrincipal principal = new CliCustomPrincipal();
	 ...
	 * 
	 * // Define a view class MyView extends NavigationView { public MyView() {
	 ...
	 * 
	 * group.addComponent(new TextField("Planet")); group.addComponent(new
	 * NumberField("Found")); group.addComponent(new Switch("Probed"));
	 * 
	...
	protected void init(VaadinRequest request) {
		...
		UI.getCurrent().getSession().setAttribute("exp", request.getParameter("exp"));
		...
		if (request.getParameter("exp") == null || request.getParameter("exp") == "") {
			...
			try {
				...
				respuesta = llamada.ejecutaPAC_SHWEB_CLIENTES__F_VALIDAR_TOKEN_SMS(token, "-1");
				...
				if (!retorno.get("CODIGOERROR").toString().equals("-3")) {
					new Notification("Acceso incorrecto", "Datos de entrada inválidos", Notification.Type.ERROR_MESSAGE,
		...
		if (entrada.equals("expediente")) {
			...
			// System.out.println("Pinemos el
			// usuaruio:"+System.getProperty("user.name").toUpperCase());
		...
		setErrorHandler(new ErrorHandler() {
			...
			public void error(com.vaadin.server.ErrorEvent event) {
				...
				new Notification("Se ha producido un error inesperado", event.getThrowable().toString(),
						Notification.Type.ERROR_MESSAGE, true).show(Page.getCurrent());
	...
	private void doLoginExpediente() {
		...
		try {
			...
			// Una vez introducido el DNI validamos que es correcto junto al
			// token de entrada
			PAC_SHWEB_CLIENTES llamada = null;
			try {
				llamada = new PAC_SHWEB_CLIENTES(service2.plsqlDataSource.getConnection());
			...
			// Recuperamos los datos del Expediente;
			try {
				respuesta = llamada.ejecutaPAC_SHWEB_CLIENTES__F_VALIDAR_TOKEN_SMS(token, dniNif.getValue());
				...
				if (!retorno.get("CODIGOERROR").toString().equals("0")) {
					new Notification("Acceso incorrecto", "Código Dni/Nif del titular incorrecto",
							Notification.Type.ERROR_MESSAGE, true).show(Page.getCurrent());
					return;
				}
			...
			UI.getCurrent().getSession().setAttribute("resoluciony",
			...
			// UI.getCurrent().getSession().setAttribute("tipousuario",
			// retLogin.get("TIPOUSUARIO"));
			// UI.getCurrent().getSession().setAttribute("posaltaexp",
			// retLogin.get("POSALTAEXP"));
			// UI.getCurrent().getSession().setAttribute("username", nomUsu + "
			// " + apeUsu);
			UI.getCurrent().getSession().setAttribute("entorno", usuEntorno);
			// UI.getCurrent().getSession().setAttribute("origen", plataforma );
			UI.getCurrent().getSession().setAttribute("origen", "BPM");

			UI.getCurrent().getNavigator().addView("CliPantallaConsultaExpediente",
					CliPantallaConsultaExpediente.class);
			// UI.getCurrent().getNavigator().addView("CliPantallaBusquedaExpedientes",
			// CliPantallaBusquedaExpedientes.class);
			UI.getCurrent().getNavigator().navigateTo("CliPantallaConsultaExpediente");
			System.out.println("Navegamos");
		} catch (CommitException e) {
			new Notification("Campos obligatorios", "Faltan campos obligatorios por entrar",
	...
	private void doLogin() {
		...
		try {
			...
			props.setProperty(OracleConnection.CONNECTION_PROPERTY_THIN_VSESSION_PROGRAM, "<<<< PWP >>>>");
			try {
			...
			if (conexionOK) {
				...
				if (!retLogin.get("CODIGOERROR").toString().equals("0")) {
					...
					new Notification("Error de usuario/password",
							"Conexión denegada.<br>" + "" + retLogin.get("TEXTOERROR"),
							Notification.Type.TRAY_NOTIFICATION, true).show(Page.getCurrent());
					// log.info(messageResolver.getMessage("logging.badlocale",
					// new Object[]{user.getIdioma(),
					// localeResolver.getLocale().toLanguageTag()}));
					...
					UI.getCurrent().getSession().setAttribute("entorno", usuEntorno);
					// UI.getCurrent().getSession().setAttribute("origen",
					// plataforma );
					...
					cargarVariablesGlobalesDatosBusqueda();
				...
				// System.out.println("Ha fallado la conexion");
				new Notification("Error de usuario/password", "Conexión denegada.<br>" + "" + msgErrorConexion,
						Notification.Type.TRAY_NOTIFICATION, true).show(Page.getCurrent());
				// log.info(messageResolver.getMessage("logging.badlocale", new
				// Object[]{user.getIdioma(),
				// localeResolver.getLocale().toLanguageTag()}));

			}

		} catch (CommitException e) {
			new Notification("Campos obligatorios", "Campos obligatorios", Notification.Type.TRAY_NOTIFICATION, true)
	...
	public void cargarVariablesGlobalesDatosBusqueda() {
		
		System.out.println("Entramos cargar variables");
		WEB_AMA llamada = null;
		try {
			llamada = new WEB_AMA(service2.plsqlDataSource.getConnection());
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		// Recuperamos los datos del Expediente;
		try {
			respuesta = llamada.ejecutaWEB_AMA__OBTENERCONTRATOS(UI.getCurrent().getSession().getAttribute("user").toString());
			Map<String, Object> mapContratos = new HashMap<String, Object>(respuesta);
			UI.getCurrent().getSession().setAttribute("contratos",mapContratos);
